//Ejercicio 5 No funciona correctamente, no se como hacer que compare con el arbol para poder asi agregar a la lista :( :(
//Ayuda Dx 
#include <iostream>
using namespace std;
...
void comparaDA(Arbol a,int num){//Compara dato si esta en arbol
	if(a != NULL){//Recorre por el arbol inorden y compara si esta el num, si lo encuentra lo agrega a la lista.
        comparaDA(a->izq,num);
        comparaDA(a->der,num);
		if(num == a->info){//Intente usar postorden y en lugar del cout ponia que comparara
			insertarL(num);//No se si realmente los agrega a la lista :( no se como hacer para ir comparando
			}
		}
	}
void compararNum(Arbol a){
	...
	cout<<endl;
	cout<<"Los numeros encontrados fueron: "<<endl;
    mostrarLista();//No se si al momento de agregar a la lista lo hace realmente y por eso no muestra nada
	
	//Se muestra el promedio
	cout<<"El promedio de la lista es de: "<< promedioLista() <<endl; //El promedio da 0 porque al no existir lista solo se muestra el 0
}
